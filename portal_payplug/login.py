...
LOGIN_URL    = "https://portal.payplug.com/auth/login"
SESSION_FILE = "session.json"
OWNER_FILE   = "session_owner.json"
TIMEOUT_MS   = 300_000   # 5 minutes max pour se connecter

SEL_USERNAME = '[data-e2e="login-form-email"]'
SEL_PASSWORD = '[data-e2e="login-form-password"]'
SEL_SUBMIT   = '[data-e2e="login-form-submit"]'
...
async def try_auto_login(page, login: str, password: str) -> bool:
    ...
    try:
        ...
        await page.wait_for_url(lambda url: "/auth/login" not in url.lower(), timeout=20000)
    ...
    try:
        ...
        try:
            await page.wait_for_url(
                lambda url: "/auth/login" not in url.lower(),
                timeout=TIMEOUT_MS,
            )
        except Exception:
            pass
